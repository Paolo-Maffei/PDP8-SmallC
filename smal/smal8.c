...
/* FILES:
     ...
     listing file suffix      -- file.lst
     object file suffix       -- file.o
...
/* version control */
#define SMALversion "SMAL8 (rev  3/12)               "
...
/* object code items (words) listed on a line */
#define listcodes       50
...
/* inside smal8, procedures for doing output format conversion */
...
static void writeoctal(FILE *f, uint32_t v, int l)
{
	/* write v as an l digit octal number on file f */
	char digits[8];
	char digit;
	int i;
...
{
	/* generate siz word value in object file based on indicated
	   symbol with the given offset from that symbol */
	if (base == abssym) {
		putc('#', obj);
		writeoctal(obj, offset, siz * 4);
	} else {
		if (offset == 0)
			putc(' ', obj);
		else {
...
/* inside smal8, procedures for symbol table management */
...
{   /* opinit */
	...
	op("RAL     ", opopr, 07004L, &i);
	op("RTL     ", opopr, 07006L, &i);
	op("RAR     ", opopr, 07010L, &i);
	...
	op("HLT     ", opopr, 07402L, &i);
	op("OSR     ", opopr, 07404L, &i);
	op("SKP     ", opopr, 07410L, &i);
	op("SNL     ", opopr, 07420L, &i);
	op("SZL     ", opopr, 07430L, &i);
	op("SZA     ", opopr, 07440L, &i);
	op("SNA     ", opopr, 07450L, &i);
...
/* inside smal8.onepass, input/output routines */
...
static void printobjectsnippet()
{
	...
	{ /* first, print location */
		...
		writeoctal(o, WITH->loc.offset, 5L);
		putc(':', o);
		listcol = 8;
		listloc = WITH->loc;
	}
	...
	do { /* list at least one code item, as many as will fit */
		...
		writeoctal(o, WITH->val.offset, WITH->form * 4);
		putc(' ', o);
		listcol = nextcol;
...
/* inside smal8.onepass, lexical analysis routines */
...
/* inside smal8.onepass, string pool and symbol table management */
                ...
                if (WITH->id != 0) {  /* have nonblank entry */
                        ...
                        &&  (WITH->val.offset == 0)) {
                                ...
                                if (WITH->val.base != abssym) {
                                        ...
                                        /* absolute values */
                                        putc('#', dmp);
                                        writeoctal(dmp, WITH->val.offset, 5);
                                }
                        }
                        putc('\n', dmp);
                }
...
/* inside smal8.onepass, utility parsing procedures */
...
/* inside smal8.onepass, procedures to parse expressions */
...
static boundval( uint32_t *v, int32_t min, int32_t max )
{
...
/* inside smal8.onepass, processing of key syntactic elements */
...
/* inside smal8.onepass, processing of external symbol linkages */
...
/* inside smal8.onepass, processing of text insertions */
...
/* inside smal8.onepass, processing of conditional directives */
...
/* inside smal8, main assembly procedure for one pass through source */
...
/* ---------------------------- */
	...
	case oppage:    /* force page alignment */
                 ...
                 * of a page!
                */
                loc.offset = (loc.offset + 0177) & 077600;
		break;
                ...
                {   int indir, i;
                    ...
                    if (expr.base == abssym) {
                        if ((expr.offset&07777) < 127) {
                            ...
                            */
                            putobj(1L, opval + indir + expr.offset, abssym);
                        } else if (loc.base == abssym) {
                            /*
                             * The reference is to an absolute address on
                    ...
                    /*
                     * Either the instruction or the operand are relocatable.
                     * Don't attempt to fully resolve the instruction yet.
                    */
                    if (allowlist) {
	...
	case opopr:
		...
		while (lex.typ == id) {
                    opcode();
                    if (optype != opopr) { /* better be another OPR */
...
/* inside smal8, procedures to do file setup and takedown */

static void getfiles(int argc, char *argv[])
{
	/*  get file name from command line, store it in global "infile"
	    suffix it with ".lst" for "outfile", ".o" for "objfile"
	    and ".d" for "dumpfile" */
	int dot, i;
	...
	outfile[dot + 1] = 'l';
/*
 * In the cross-development environment, .l files are taken as sources
 * written in lex, then the corresponding .c files get clobbered by make.
 * So here we use ".lst" instead of just ".l".
*/
	outfile[dot + 2] = 's';
	outfile[dot + 3] = 't';
	outfile[dot + 4] = 0;
