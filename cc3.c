...
constexpr(val) int *val; {
  ...
  if(con == 0) error("must be constant expression");
  return con;
  }
...
down2(oper, oper2, level, is, is2)
  ...
  if(is[TC]) {                    /* consant op unknown */
    ...
    gen(GETw2n, is[CV] << dble(oper, is2, is));
    }
    ...
    if(is2[TC]) {                 /* variable op constant */
      ...
      if(oper == ADD12) {         /* commutative */
        gen(GETw2n, is2[CV] << dble(oper, is, is2));
        }
      else {                      /* non-commutative */
        gen(MOVE21, 0);
        gen(GETw1n, is2[CV] << dble(oper, is, is2));
        }
      }
    else {                        /* variable op variable */
      gen(POP2, 0);
      if(dble(oper, is, is2)) gen(DBL1, 0);
      if(dble(oper, is2, is)) gen(DBL2, 0);
      }
    }
  if(oper) {
